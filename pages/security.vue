<template>
  <div>
    <div>
      <MainContent>
        <template v-slot:title>{{ security.title }}</template>
        ...
        <template v-slot:content>
          <div
            ...
            v-html="$md.render(security.content)"
          ></div>
        </template>
      </MainContent>
    </div>
...
export default {
  async asyncData({ params }) {
    let res = await axios.get(`https://reneil.app/strapi/securities`, {
      // headers: {
      //   Authorization:
