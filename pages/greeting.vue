<template>
  <div>
    <div>
      <MainContent>
        <template v-slot:title>{{ greeting.title }}</template>
        ...
        <template v-slot:content>
          <div
            ...
            v-html="$md.render(greeting.content)"
          ></div>
        </template>
      </MainContent>
    </div>
...
export default {
  async asyncData({ params }) {
    let res = await axios.get(`https://reneil.app/strapi/greetings`, {
      // headers: {
      //   Authorization:
